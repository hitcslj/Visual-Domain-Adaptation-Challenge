...
class SegCLIP(nn.Module):
    ...
    """
    def __init__(self, train_clip=False):
        """
        初始化函数
        ...
        """
        super().__init__()
        self.model = CLIPDensePredT(version='ViT-B/16', reduce_dim=64, grad=train_clip)
    ...
    def forward(self, x):
        ...
        prompts = ['background', 'rigid_plastic', 'cardboard', 'metal', 'soft_plastic']
        out = self.model(x.repeat(5, 1, 1, 1), prompts)[0]
...
class CLIPDenseBase(nn.Module):
    ...
    def __init__(self, version, reduce_cond, reduce_dim, prompt, n_tokens, grad):
        super().__init__()

        import clip

        # prec = torch.FloatTensor
        self.clip_model, _ = clip.load(version, device='cpu', jit=False, download_root='/data/challenge/pretrained_weights/')
    ...
    def visual_forward(self, x_inp, extract_layers=(), skip=False, mask=None, grad=False):
...
class CLIPDensePredT(CLIPDenseBase):
    ...
    def __init__(self, version='ViT-B/32', extract_layers=(3, 6, 9), cond_layer=0, reduce_dim=128, n_heads=4, prompt='fixed', 
                 ...
                 add_calibration=False, rev_activations=False, trans_conv=None, n_tokens=None, grad=False):
        
        super().__init__(version, reduce_cond, reduce_dim, prompt, n_tokens, grad)
        # device = 'cpu'
