...
class FCNs(nn.Module):
    ...
    def __init__(self, pretrained_net, n_class, p=0):
        super().__init__()
        self.n_class = n_class
        self.pretrained_net = pretrained_net
        self.relu    = nn.ReLU(inplace=True)
        ...
        self.bn5     = nn.BatchNorm2d(32)
        self.classifier = nn.Conv2d(32, n_class, kernel_size=1)
        self.drop_out = nn.Dropout3d(p=p)
    ...
    def forward(self, x):
        ...
        score = self.bn1(self.relu(self.deconv1(x5)))     # size=(N, 512, x.H/16, x.W/16)
        score = F.interpolate(score, size=(x4.shape[2], x4.shape[3]))
        score = score + x4                                # element-wise add, size=(N, 512, x.H/16, x.W/16)
        score = self.drop_out(score)
        score = self.bn2(self.relu(self.deconv2(score)))  # size=(N, 256, x.H/8, x.W/8)
        score = F.interpolate(score, size=(x3.shape[2], x3.shape[3]))
        score = score + x3                                # element-wise add, size=(N, 256, x.H/8, x.W/8)
        score = self.drop_out(score)
        score = self.bn3(self.relu(self.deconv3(score)))  # size=(N, 128, x.H/4, x.W/4)
        score = F.interpolate(score, size=(x2.shape[2], x2.shape[3]))
        score = score + x2                                # element-wise add, size=(N, 128, x.H/4, x.W/4)
        score = self.drop_out(score)
        score = self.bn4(self.relu(self.deconv4(score)))  # size=(N, 64, x.H/2, x.W/2)
        score = F.interpolate(score, size=(x1.shape[2], x1.shape[3]))
